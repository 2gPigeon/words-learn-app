import type { Deck, IrregularVerbDeck, IrregularVerbItem, WordItem } from '../types'
...
function selectPrintableItems<T>(items: T[], count: number) {
...
function renderIrregularVerbRows(
  ...
  return Array.from({ length: rowCount }, (_, index) => {
    const item = items[index]
    const meaning = item
      ? `<span class="sheet__number">${index + 1}.</span>${escapeHtml(item.meaning)}`
      : ''
    const word = item && mode === 'answer-key' ? escapeHtml(item.word) : ''
    const past = item && mode === 'answer-key' ? escapeHtml(item.past) : ''
    const pastParticiple =
      item && mode === 'answer-key' ? escapeHtml(item.pastParticiple) : ''

    return `
      <tr>
        <td class="sheet__meaning">${meaning}</td>
        <td class="sheet__word">${word}</td>
        <td class="sheet__past">${past}</td>
        <td class="sheet__past-participle">${pastParticiple}</td>
      </tr>
    `
  }).join('')
}

function buildIrregularVerbPrintableHtml(
    ...
    <main class="page">
      ...
      <section class="page__halves">
        <section class="sheet sheet--answers" aria-label="Answer key">
          ...
          <table>
            <colgroup>
              <col class="sheet__col--meaning" />
              <col class="sheet__col--word" />
              <col class="sheet__col--past" />
              <col class="sheet__col--past-participle" />
            </colgroup>
            <thead>
              <tr>
                <th>意味</th>
            ...
            <tbody>
              ${renderIrregularVerbRows(items, PRINTABLE_ROW_COUNT, 'answer-key')}
            </tbody>
          </table>
        </section>
        ...
        <section class="sheet sheet--test" aria-label="Printable irregular verb test">
          <div class="sheet__heading">
            <h2>テスト</h2>
          </div>
          <table>
            ...
            <tbody>
              ${renderIrregularVerbRows(items, PRINTABLE_ROW_COUNT, 'test')}
            </tbody>
          </table>
        </section>
      </section>
...
export function openIrregularVerbPrintableTest(deck: IrregularVerbDeck) {
  if (deck.items.length === 0) {
